               ...
               {item.description}
                  ...
                  <Button
                    ...
                    >
                      ...
                      { item.comment ? (
                       ...
                       {item.comment.map( (comment) => ( 
                          <div className='ml-3 mb-2' key={comment._id}> <span className="font-weight-bold">{comment.user}</span> {comment.content} 
                       <div className="small"> 
                    ...
                    >
                      ...
                      {item.replies ? (
                        <div>
                      {item.replies.parent === item.comment._id  ? 
                        (<div>
                        {item.replies.map( (reply) => ( 
                          <div className='ml-3 mb-2' key={reply._id}> <span className="font-weight-bold">{reply.user}</span> {reply.content} 
                          </div>
                          ))}
                        </div>):null}
